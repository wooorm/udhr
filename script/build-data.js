  ...
  for (property in object) {
    ...
    if (property === '$') {
      for (nestedProperty in value) {
        ...
        clean[nestedProperty] = cleanXMLJSON(value[nestedProperty], nestedProperty, allowDirty);
      }
    } else {
...
function cleanString(value, key, allowDirty) {
  ...
  if (
    ...
    if (key === 'title' && !(/preamble/i.test(value))) {
      ...
      console.log('Unwrapping title:            "' + value + '" > "' + newValue + '"');
      value = newValue;
    } else if (!allowDirty) {
    ...
    .map(function (declaration) {
      ...
      if (cleanDeclaration.code === 'nku') {
        cleanDeclaration.hasTXT =
          Boolean(readFile('data/udhr-txt/udhr_' + filename + '.txt', 'utf-8'));

        if (cleanDeclaration.hasTXT) {
          console.log('Warning: NKU error was fixed. The special code (see warning) should be removed.');
        }
      } else {
